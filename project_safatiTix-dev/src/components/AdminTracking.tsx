...
const AdminTracking: React.FC<AdminTrackingProps> = ({ 
  scheduleId,
  routeFrom,
  routeTo,
  busPlate,
  ...
  useEffect(() => {
    const initializeTracking = async () => {
      try {
        ...
        socket.on('connect', () => {
          console.log('✅ Super Admin connected to tracking server');
          setConnectionStatus('connected');
          
          // Join the schedule room as super admin
          socket.emit('admin:joinSchedule', { scheduleId });
        });

        socket.on('admin:joinedSchedule', (data) => {
          console.log('✅ Super Admin successfully joined tracking:', data);
        });
  ...
  return (
    <div className="space-y-4">
      {/* Admin Badge */}
      <div className="bg-gradient-to-r from-red-600 to-red-700 text-white rounded-lg p-3 shadow-md">
        ...
        >
          ...
          {currentLocation && (
            ...
            >
              <Popup>
                <div className="text-center">
                  <strong className="block mb-2">System Bus Tracking</strong>
                  {companyName && <p className="text-sm mb-1">Company: {companyName}</p>}
